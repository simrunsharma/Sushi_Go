...
def test_switch_hands():
    ...
    assert (
        game.player1.hand == pre_switch_hand_p2
    ), "Player 1's hand should now have Player 2's cards"
    assert (
        game.player2.hand == pre_switch_hand_p1
    ), "Player 2's hand should now have Player 1's cards"
...
def test_calculate_final_score():
    ...
    assert (
        score == 15
    ), "The score calculation should correctly calculate based on the card set rules"
...
def test_conduct_round():
    ...
    game.conduct_round()
    assert (
        len(game.player1.hand) == 0 and len(game.player2.hand) == 0
    ), "Both players should have empty hands after a round"
...
def test_assign_cards():
    ...
    assert (
        len(deck.cards) == 1
    ), "Deck should have 1 card left after assignment"
...
def test_play_best_card():
    ...
    class SimpleMaximizer:
        ...
        def select_best_card(self):
            return self.player.hand[
                0
            ]  # Simulate always picking the first card
    ...
    player.play_best_card = lambda table: SimpleMaximizer(
        player, table
    ).select_best_card()

    best_card = player.play_best_card(table)
    print(
        "Played best card:", best_card.card_type
    )  # Printing the type of the best card played
    print(
        "Remaining hand:", [card.card_type for card in player.hand]
    )  # Printing remaining cards in hand
    assert (
        best_card.card_type == "Sashimi"
    ), "Should play Sashimi as it is selected as best card"
    print(
        "Number of cards remaining in hand (Expected: 2):", len(player.hand)
    )  # Print the actual number of cards
